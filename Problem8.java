import java.util.ArrayList;

public class Problem8{
    
    public static void main (String[] args) {
        String input = "";
        ArrayList<Integer> arrIn = new ArrayList<Integer>();
        arrIn = toArrayList(input);
        long product = maxProduct(arrIn, 13);
        System.out.println(product);
    }
    
    //Convert the input String into an ArrayList
    public static ArrayList<Integer> toArrayList(String str){
        String[] str2 = str.split("");
        ArrayList<Integer> str3 = new ArrayList<Integer>();
        for(int i = 1; i<str2.length; i++){
            str3.add(Integer.parseInt(str2[i]));
        }
        return str3;
    }
    
    
    public static long maxProduct(ArrayList<Integer> in, int adj){
        long maxProduct = 0;
        //Should check that the input ArrayList has at least 13 elements
        for(int i = 0; i<in.size()-adj; i++){
            long result = 1;
            for(int j = i; j < i+adj; j++){
                if(in.get(j).equals(0)){
                    i+=j-i;
                    result = 1;
                    break;
                }
                else{
                    result *= in.get(j);
                }
            }
            if(result>maxProduct){
                maxProduct = result;
            }
        }
        return maxProduct;
    }
    
}